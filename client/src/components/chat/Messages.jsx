...
const Message = ({ msg, type, time }) => {
  return (
    ...
    >
      <div className={`flex flex-col ${type === "user" && "items-end"} max-w-[70%] md:max-w-[60%]`}>
        <div
        ...
        <span className={`text-[10px] md:text-xs mt-2 text-[#949494]`}>{time}</span>
      </div>
    </motion.div>
  );
};
...
const Messages = ({ messages }) => {
  ...
  return (
    <div className="space-y-4 max-h-[400px] md:max-h-[500px]  overflow-y-scroll scrollbar-hide mt-12">
      {messages.length ? (
